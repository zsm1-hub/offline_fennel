function ext_tracers_ini(ininame,grdname,seas_datafile,ann_datafile,...
                         dataname,vname,type,tini,transform,stretch,cycle);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
%ttini=mod(tini,cycle)  add cyp 2022 3 5

ll=find(T<=tini);

if (max(size(ll)) ~= 0) % size(ll,1)
 l=ll(max(size(ll)))
else
 l=1;
end
...
%
zroms=zlevs(h,0.*h,theta_s,theta_b,hc,N,'r',vtransform); %lwx, added vtransform for new verssion
%zroms=zlevs_wj(h,0.*h,theta_s,theta_b,hc,N,'r',transform,stretch);
if type=='u'
