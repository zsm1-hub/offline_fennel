function add_chla(climfile,gridfile,seas_datafile,cycle,Roa,transform,stretch);
...
redef(nc);
nc('chlo_time') = tlen;
nc{'chlo_time'} = ncdouble('chlorophyll_time') ;
nc{'chlorophyll'} = ncdouble('chlorophyll_time','s_rho','eta_rho','xi_rho') ;
%
nc{'chlorophyll_time'}.long_name = ncchar('time for chlorophyll');
nc{'chlorophyll_time'}.long_name = 'time for chlorophyll';
nc{'chlorophyll_time'}.units = ncchar('day');
nc{'chlorophyll_time'}.units = 'day';
if cycle~=0
  nc{'chlorophyll_time'}.cycle_length = cycle;
end
%
nc{'chlorophyll'}.long_name = ncchar('Chlorophyll');
nc{'chlorophyll'}.long_name = 'Chlorophyll';
nc{'chlorophyll'}.units = ncchar('mg C');
nc{'chlorophyll'}.units = 'mg C';
nc{'chlorophyll'}.fields = ncchar('chlorophyll, scalar, series');
nc{'chlorophyll'}.fields = 'chlorophyll, scalar, series';
%
endef(nc);
%
% Record the time
%
nc{'chlorophyll_time'}(:)=t*365.25/12.; % if time in month in the dataset !!!
%
...
%
  zroms=zlevs(h,0.*h,theta_s,theta_b,Tcline,N,'r',transform,stretch);
  disp(['Add_chla: vertical ',...
  'extrapolation of chlorophyll'])
  chlaroms=extr_chlo(surfchlaroms,zroms);
  nc{'chlorophyll'}(l,:,:,:)=chlaroms;
end
close(nc);
close(ncseas);
