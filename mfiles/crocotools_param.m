...
%
CROCO_title  = 'TWS_L1_offline';
CROCO_config = 'TWS_L1_offline';
%
% Grid dimensions:
%
lonmin = 116;   % Minimum longitude [degree east]
lonmax = 123;   % Maximum longitude [degree east]
latmin = 20;   % Minimum latitudeF  [degree north]
latmax = 27;   % Maximum latitude  [degree north]

%closed by zsm,because I already have a curved grid
...
%  theta_s: 1-7  the large the high re in surface
%  theta_b:  <4  the large the high re in bottom
%  Tcline : if tcline>>H the layer is uniform, if tcline<< H high in surface
theta_s    =  6;     %  3.;
theta_b    =  6;     %  0.4;
hc         =  2;   %  10;
vtransform =  2.; % s-coordinate type (1: old- ; 2: new- coordinates)
                  % ! take care to define NEW_S_COORD cpp-key in cppdefs.h
stretching = 4. ; % added by lwx 
% ATTENTION!!! : I have modified Vstretching from 1 to 4 in create_inifile.m ,
% create_bryfile.m and create_climfile.m to fit romsv974. xmd.
%
...
%
hmax = 5000; %5000 -origin change by zsm
%
...
% topography.
%
n_filter_final=2;       % modified by xmd from 2 to 4
%
...
%
coastfileplot = 'coastline_h.mat';
coastfilemask = 'coastline_h_mask.mat';
%
...
Roa=0;
%
interp_method = 'spline';         % Interpolation method: 'linear' or 'cubic' modified by xmd
%
...
%
CROCO_files_dir=[RUN_dir,'CROCO_FILES/TWS2/'];
%
...
%
FORC_DATA_DIR = '/export/home/zsm/work/newproject/croco_tools/DATA/';
...
%
%grdname  = [CROCO_files_dir,'TWS_grd2.nc'];
grdname  = [CROCO_files_dir,'TWS2_L1_grid.nc'];
frcname  = [CROCO_files_dir,'croco_ncep_frc.nc.1'];  % combination frc file name.xmd
blkname  = [CROCO_files_dir,'croco_ncep_blk.nc.1'];  % combination blk file name.xmd
clmname  = [CROCO_files_dir,'croco_clm.nc'];
bryname  = [CROCO_files_dir,'croco_myocean_bry2018.nc'];
ininame  = [CROCO_files_dir,'bio_TWS_L1_ini.nc'];
%grdname  = [CROCO_files_dir,'chinav2.nc'];
frcname  = [CROCO_files_dir,'croco_ncep_frc.nc'];  % combination frc file name.xmd
blkname  = [CROCO_files_dir,'croco_ncep_blk.nc'];  % combination blk file name.xmd
clmname  = [CROCO_files_dir,'croco_clm.nc'];
bryname  = [CROCO_files_dir,'croco_myocean_bry2018.nc'];
%ininame  = [CROCO_files_dir,'croco_myocean_ini2018.nc'];
%ininame  = [CROCO_files_dir,'croco_ini.nc'];
%bryname  = [CROCO_files_dir,'croco_bry_MYOCEAN_.nc'];
%ininame  = [CROCO_files_dir,'croco_ini_MYOCEAN_Y2013M1.nc'];
...
biolZbryname = [CROCO_files_dir,'croco_bry_Z.nc']; % added by lwx ,for biology boundary data processing
%
...
%
obc = [1 1 1 0]; % open boundaries (1=open , [S E N W]) %%change by zsm, open the N bc
%
...
makenpzd   = 1;   % initial and boundary data for NChlPZD and N2ChlPZD2 models
...
Yorig         = 2000;          % reference time for vector time
			       ...
			       % if you modify Yorig, 
			       % grep 1900 under tools fold. xmd.
%
...
tini=datenum(Ymin,Mmin,Dmin)-datenum(2000,1,1);
% below added by xmd using in make_OGCM.m
% as filename and tindex.
tmax=datenum(Ymax,Mmax,Dmax)-datenum(Ymin,Mmin,Dmin)+1;
dt=1;                      % dt represent the time interval,unit: day
trange=[1:dt:tmax];
if max(trange)<tmax
  trange=[trange tmax];
end
OGCM_time=trange+datenum(Ymin,Mmin,Dmin)-1;
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
Get_My_Data   = 1;     % 1: use local datasets
%
% Provide corresponding directory paths
% ncep_url=My_NCEP_dir when using local data.xmd.
%
if NCEP_version  == 1;
  My_NCEP_dir  = [DATADIR,'NCEP_REA1/'];
elseif NCEP_version  == 2;
% My_NCEP_dir  = [DATADIR,'NCEP_REA2/'];
  My_NCEP_dir  = './downloaddata/NCEP/00_bat/';
...
elseif NCEP_version  == 2;
  NCEP_dir= ['./downloaddata/NCEP/00_bat/'];
elseif NCEP_version  == 3;
...
itolap_ncep  = 0;      % 8 records for 4-daily NCEP.xmd.8 to 0.
%
%--------------------------------------------------
...
%-----------------------
%
OGCM        = 'MYOCEAN';        % Select the OGCM: SODA, ECCO xmd: MYOCEAN
%
% OGCM_dir    = [FORC_DATA_DIR,OGCM,'_',CROCO_config,'/']; % OGCM data directory
OGCM_origin = ['/powerleader/data/myocean_60N/2019/'];          % original OGCM data xmd
OGCM_dir    = ['/sugon7/zsm/croco_tools_xmd1204/CROCO_FILES/TWS/'];  % zsm
OGCM_dir    = ['/pacific4/user/zsm/'];  % zsm
...
%
itolap_a    = 0;   % before   modified by xmd from 2 to 0
itolap_p    = 0;   % after    modified by xmd from 2 to 0
%
%--------------------------
