...
y=ncseas{'Y'}(:);
datatime=ncseas{'T'}(:);
datatime=datatime*365.25/12.;  % !!! if the time in the dataset is in months !!!
...
%imax=find(x==max(x)); % if max(x)<maxlon
...
%
  ...
  zroms=zlevs(h,zeta,theta_s,theta_b,hc,N,'r',vtransform); %lwx
...
close(nc);
disp([ndims(chlaroms) ndims(nc{'chlorophyll'}(:))]);
[AA,BB,CC]=size(chlaroms)
[AA,BB]=size(nc{'chlorophyll'}(:))
%disp(chlaroms)
%close(ncseas);
return
